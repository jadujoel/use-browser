...
beforeAll(() => { events.push("rootBeforeAll"); });
afterAll(() => { events.push("rootAfterAll"); });
beforeEach(() => { events.push("rootBeforeEach"); });
afterEach(() => { events.push("rootAfterEach"); });
...
describe("nested", () => {
  beforeEach(() => { events.push("nestedBeforeEach"); });
  afterEach(() => { events.push("nestedAfterEach"); });
  ...
  test("inherits root beforeEach and runs nested beforeEach after", () => {
    ...
    expect(events.filter((e) => e === "rootBeforeEach").length).toBeGreaterThanOrEqual(2);
...
test("second root test sees afterEach from previous test", () => {
  ...
  expect(events.filter((e) => e === "rootAfterEach").length).toBeGreaterThanOrEqual(2);
