...
const matchersFor = (actual: unknown, negated: boolean): Matchers => {
  ...
  const base: CoreMatchers = {
    ...
    toBeInstanceOf: (ctor) => {
      const passed = typeof ctor === "function" && actual instanceof (ctor as new (...args: unknown[]) => unknown);
      check(
        passed,
        negated,
        "toBeInstanceOf",
    ...
    toContain: (item) => {
      ...
      else if (Array.isArray(actual)) passed = actual.some((a) => Object.is(a, item));
      else if (actual instanceof Set) passed = actual.has(item);
      else if (actual !== null && actual !== undefined && Symbol.iterator in (actual as object)) {
        for (const v of actual as Iterable<unknown>) {
          if (Object.is(v, item)) { passed = true; break; }
        }
      }
      check(
        passed,
    ...
    toThrow: (matcher) => {
      if (typeof actual !== "function") {
        fail("toThrow", actual, matcher, `expected a function, got ${fmt(actual)}`);
      }
      let thrown: unknown;
      let didThrow = false;
      try {
      ...
      if (negated) {
        if (didThrow) {
          fail("toThrow", thrown, matcher, `expected function not to throw, but it threw ${fmt(thrown)}`);
        }
        return;
      }
      if (!didThrow) {
      ...
      if (typeof matcher === "string") {
        if (!errMessage.includes(matcher)) {
          fail("toThrow", errMessage, matcher, `expected error message to contain ${fmt(matcher)}, got ${fmt(errMessage)}`);
        }
      } else if (matcher instanceof RegExp) {
        if (!matcher.test(errMessage)) {
          fail("toThrow", errMessage, matcher, `expected error message to match ${matcher}, got ${fmt(errMessage)}`);
        }
      } else if (typeof matcher === "function") {
        if (!(thrown instanceof (matcher as new (...args: unknown[]) => unknown))) {
          const ctorName = (matcher as { name?: string }).name ?? "<ctor>";
          fail("toThrow", thrown, matcher, `expected error to be instance of ${ctorName}, got ${fmt(thrown)}`);
        }
      }
    },
    toBeGreaterThan: (n) => {
    ...
    toMatch: (pattern) => {
      ...
      const passed = pattern instanceof RegExp ? pattern.test(str) : str.includes(pattern);
      check(
        passed,
        negated,
        "toMatch",
