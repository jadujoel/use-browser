...
const RUNTIME_ENTRY = new URL("./browser/runtime-entry.ts", import.meta.url).pathname;
 ...
 */
export const buildBundle = async (options: BuildBundleOptions): Promise<string> => {
  const result = await Bun.build({
    entrypoints: [RUNTIME_ENTRY],
    target: "browser",
    format: "esm",
    sourcemap: "inline",
    plugins: [bunTestPlugin({ userFile: options.userFile, shimPath: SHIM_PATH })],
  });
  if (!result.success) {
 ...
 */
export const buildHtmlPage = async (options: BuildBundleOptions): Promise<string> => {
